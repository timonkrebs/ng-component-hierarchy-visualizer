...
import { extractRoutesFromTS, findImportPath, findAllImports } from './route.helper.js';
...
const handleLoadChildren = (route) => {
    ...
    if (extractedRoutes?.length) {
        ...
        const imports = findAllImports(routesFileContent);
        const moduleImportMatch = imports.find(match => {
            const path = match.path;
            return path.match(/routing/) || path.match(/routes/);
        });

        const originalModulePath = moduleImportMatch?.path ?? imports.find(i => i.path)?.path;
        if (!originalModulePath) {
             throw new Error('Could not find routing module import');
        }
